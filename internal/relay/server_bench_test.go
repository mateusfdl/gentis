...
// Phase F benchmarks for the relay. Each bench runs under `heap` and
// `arena` sub-benchmarks. The relay requires an upstream gRPC server to
// connect to; we spin up a vanilla one per bench and point the relay at
// it. The relay router is overridden to route ALL channels locally so
// the benches measure relay-local delivery (the arena's domain) without
// the confounding variable of the upstream round-trip.

// benchMode selects which per-session state backend the relay uses.
type benchMode int
...
// startBenchUpstream spins up a bare gRPC server (no metrics, no arena)
// for the relay to dial.
func startBenchUpstream(b *testing.B) (string, func()) {
	b.Helper()
	...
	if err := srv.Start(); err != nil {
		b.Fatalf("upstream Start: %v", err)
	}
	return addr, func() { srv.Stop() }
}

func startBenchRelay(b *testing.B, mode benchMode) (string, func()) {
	b.Helper()
	...
	upstreamAddr, stopUpstream := startBenchUpstream(b)
	...
	opts := []Option{
		WithListenAddr(addr),
		WithUpstream(upstreamAddr, "bench-token"),
		WithBufferSize(256),
	...
	// Route everything locally. Must be set before Start.
	...
	if err := r.Start(); err != nil {
		stopUpstream()
		b.Fatalf("relay Start: %v", err)
	}
	// Let the relay establish the upstream stream.
...
func BenchmarkRelayConnectionMemory(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
		for _, n := range []int{100, 1000} {
			b.Run(fmt.Sprintf("N=%d", n), func(b *testing.B) {
				addr, stop := startBenchRelay(b, mode)
				defer stop()
...
func BenchmarkRelayConnectionChurn(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
		addr, stop := startBenchRelay(b, mode)
		defer stop()
...
func BenchmarkRelayMessageLatency(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
		addr, stop := startBenchRelay(b, mode)
		defer stop()
...
func BenchmarkRelayMessageThroughput(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
...
func BenchmarkRelayGCPause(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
...
func BenchmarkRelayGCPauseUnderTraffic(b *testing.B) {
	benchEachMode(b, func(b *testing.B, mode benchMode) {
		for _, n := range []int{1000} {
			b.Run(fmt.Sprintf("N=%d", n), func(b *testing.B) {
				addr, stop := startBenchRelay(b, mode)
